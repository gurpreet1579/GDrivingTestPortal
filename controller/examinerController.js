...
module.exports.schedule = (req, res) => {
    ...
    User.find( { appointmentType: { $in: ['g', 'g2'] },  userType: 'Driver'}, (error, users) => {
        if (users.length > 0) {
...
module.exports.filteredSchedule = (req, res) => {
    const { appointmentType } = req.body;
        
    User.find( { appointmentType: appointmentType,  userType: 'Driver'}, (error, users) => {
        if (users.length > 0) {
          console.log(users) ;
          res.render('schedule', { session: req.session, user: users });
        } else {
...
module.exports.scheduledDriver = async (req, res) => {
        ...
        await User.find( { userName : user}, (error, users) => {
            if (users.length > 0) {
              ...
              Appointment.find( {_id : foundUser.appointmentId  }, (err , appointment)=>{
                if(err) {
                 ...
                 if( appointment.length != 0  ){
                   return res.render('scheduledDriver', { session: req.session, user: foundUser , appointment :appointment[0] });
                  }else{
              ...
              console.log('no scheduled users found');
              return res.render('scheduledDriver', { session: req.session, user: [] , appointment :[] });
            }
